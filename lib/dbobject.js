 ...
 *     'defaultValue': <a value or a function returning a value that will be used if the property is undefined ( not just null )
 *     function is passed>
 *
...
DbObjectType.create = function(fullOptions) {
    ...
    if (Object.keys(fullOptions).length == 0) {
        throw new Meteor.Error(500,'No options in call to DbObjectType.create');
    }
    if (!_.isEmpty(_.omit(fullOptions, CreateKnownOptionKeys))) {
        throw new Meteor.Error(500,
            "invalid option(s) to DbObjectType.create:",
            _.omit(fullOptions, CreateKnownOptionKeys)
        );
    }
    ...
    function processPropertyDefinition(propertyDef, propertyName) {
        ...
        if (!_.has(propertyDefinition, 'writable') && !_.has(propertyDefinition, 'get') && !_.has(propertyDefinition, 'set')) {
    ...
    _.extend(properties, {
        ...
        // HACK should really be defined in the list of object properties via handleStringOrObjectDefinition()
        // HACK: avoids the id being saved twice in the database. really need ability to make a property as not to be saved in server db.
        id: {
            get : function() {
                return this._id;
            },
    ...
    if ( extensions != null ) {
        ...
        if (_.isArray(resolvedExtensions)) {
            console.warn(subClassType,typeName(),": extensions is an array - usually intended to be an object");
        }
    ...
    if ( databaseDefinition != null && dbCollection == null) {
        if (databaseDefinition.databaseType == null || databaseDefinition.databaseType === 'mongo') {
            databaseDefinition.databaseTable = dbCollection = new Mongo.Collection(databaseDefinition.databaseTableName, {
    ...
    if ( dbCollection != null) {
        ...
        var databaseExtensions = {
            ...
            /**
             * Make sure that the update does not replace the content of the document by forcing the use of $set.
             * if there are any non '$' keys in changedValues.
             *
            ...
            updateOneById:function(id, changedValues) {
                ...
                debugger;
                var result = subClassType.databaseTable.update({_id:id}, updateDirective);
                return result;
            },
        ...
        convenienceMethodNames.push('find');
        ...
        for (var i in indexedProperties) {
            ...
            _.each(['findOneBy', 'findBy', 'findFetchBy', 'findRequiredOneBy'], function(baseFunctionName) {
...
DbObjectType.prototype = {
    ...
    fromJSONValue: function(specialHandling, rawJson) {
        var self = this;
        // we iterate on the defined property names not the properties in the rawJson, because
        // an attacker could send an object with a lot of properties and by following a whitelist approach
        // we avoid the 'attacker-is-smarter-than-pat' problem.
        ...
        // now apply defaults, now that all the explicit values are assigned ( allows some defaults to be based on other values )
    ...
    upsertFromUntrustedKnownOptionKeys: Object.freeze([
        ...
        'revision',
        // passed to _revisionSave; don't do the save if target isn't the head of a revision
        // sequence.
        'revisionHeadOnly'
    ]),
    ...
    upsertFromUntrusted: function(optionsParam) {
        ...
        if (!_.isEmpty(_.omit(options, this.upsertFromUntrustedKnownOptionKeys))) {
            var msg = [
                "Was correct object passed to invalid option to upsertFromUntrusted?",
                "upsertFromUntrusted passed object with these keys:",
                _.keys(_.omit(options, this.upsertFromUntrustedKnownOptionKeys)),
                "expected only:",
                this.upsertFromUntrustedKnownOptionKeys
            ].join(' ');
        ...
        if (options.revision) {
            target._revisionSave(options.revisionHeadOnly);
        } else {
            target._save();
        }
        return target;
    ...
    /**
     * TO_DMR: when should onlyReviseHead be false?  seems like we should always want that behavior.
     *   Need explanation of why. we just have an obvious 'what' comment here.
     * save an object, keeping a copy of the old version
     * @param onlyReviseHead: roll back 'transaction' if the target object is not the head of a
     * sequence of revisions
     */
    _revisionSave: function(onlyReviseHead) {
        'use strict';
        var self = this;
        ...
        if (numUpdates == 0 && onlyReviseHead) {
            self.error('failed numUpdates == 0 && onlyReviseHead');
        ...
        if ( this._id ) {
            var affectedDocumentCount = this.databaseTable.update(
                ...
                function(error, affectedDocuments) {
                    if (error) {
                        self.error("Failed to update. error=",error);
                        debugger;
                    }
                }
            );
            if ( affectedDocumentCount == 0 ) {
                self.error("No documents found with id");
                debugger;
            }
        } else {
