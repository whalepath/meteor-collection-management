 ...
 * @param propertiesParam - declare the properties that the EJSON code will care about. can be
...
DbObjectType.createSubClass = function(subClassTypeParam, typeNameParam, propertyNamesParam, databaseTableNameParam, extensionsParam, privatePropertiesParam) {
    ...
    var typeNameArg = arguments[argumentIndex++], typeNameFn;
    if ( typeof typeNameArg === 'function') {
    ...
    handleStringOrObjectDefinition.call(this, propertyNamesArg, processPropertyDefinition, false);
...
DbObjectType.prototype = {
                    ...
                    if (!jsonFn) {
                        ...
                        if (jsonHelperObj != null) {
                            ...
                            if (typeof jsonHelperObj === 'string') {
                                ...
                                if ( jsonHelper == null ) {
                                    throw new Meteor.Error(500, that.typeName(),'.',propertyName,'.jsonHelper=', jsonHelperObj, " which isn't a global var name or register jsonHelper");
                                }
                            } else {
                    ...
                    if (!jsonFn) {
                        ...
                        if (jsonHelperObj != null) {
                            ...
                            if (typeof jsonHelperObj === 'string') {
                                jsonHelper = DbObjectType.prototype.dbObjectTypes[jsonHelperObj] || DbObjectType.prototype.jsonHelpers[jsonHelperObj] || _.getGlobal()[jsonHelperObj];
                                if ( jsonHelper == null ) {
                                    throw new Meteor.Error(500, that.typeName(),'.',propertyName,'.jsonHelper=', jsonHelperObj, " which isn't a global var name");
                                }
                            } else {
                                jsonHelper = jsonHelperObj;
                            }
     ...
     *
     * obj.upsertFromUntrusted(); - in this case the obj is assumed to be from the client. Only the values allowed from the client
     * are updated. The returned object is true server about.
     *
     * TODO: security check on who is allowed to update/save this object.
     * TODO: DMR: Can we put the lookupParam into options?
     ...
     */
    upsertFromUntrusted: function(clientObjParam, lookupParam, optionsParam) {
        'use strict';
        var knownOptionKeys = ['forcedValues', 'frozenKeys', 'updateOnly'];
        var clientObj, lookup, options;
        var argIndex = 0;
        var effectiveArgLength;
        for (effectiveArgLength = arguments.length; effectiveArgLength > 0 && arguments[effectiveArgLength-1] == null; effectiveArgLength--){}
        ...
        if ( this instanceof DbObjectType && this.constructor.prototype !== this) {
            if ( arguments.length > 2) {
                throw new Meteor.Error(500, "only 0,1,2 arguments when calling as instance method.");
            }
            ...
            case 0:
                ...
                break;
            case 1:
                clientObj = arguments[argIndex++];
                if (!_.isObject(clientObj) || _.isEmpty(clientObj)) {
                    // really the same as the no-arg case - but yet we have this null.
                    throw new Meteor.Error(500, 'if only one argument supplied with a instance call, it must be a non-empty object.');
                }
                options = {};
                break;
            case 2:
            ...
            // but because we are calling upsertFromUntrusted() on an instance, the instance itself is untrusted.
            // so we pickClient on this.
        } else {
            if ( arguments.length > 3 || effectiveArgLength == 0) {
                throw new Meteor.Error(500, "only 1,2,3 arguments when calling as instance method. arguments.length="+
                    arguments.length+" and effectiveArgLength (trailing nulls trimmed)="+effectiveArgLength);
            }

            clientObj = arguments[0];
            lookup = arguments[1];
            options = arguments[2] || {};
            if (_.isEmpty(clientObj)) {
                // no changes to apply. But this is not really an error, just a case of a client being lazy about
                // sending an empty object. (also could happen for other reasonable reasons)
        ...
        if ( lookup != null && _.isEmpty(lookup)) {
            ...
            // HOWEVER, we check for empty lookup because we don't want to allow a error in creating the lookup object to result
            // in an update that alters every object in the database.
            throw new Meteor.Error(500, "upsertFromUntrusted: lookup is {} - and thus would result in modifying all documents or a random single document.");
        }

        // The object to update (or insert if set to null).
        // NOTE: 'this' may have come from a client (untrusted) or loaded from the server database (trusted).
        // The combinations to reliable determine what is the best course of action are problematic and subject to developer
        // confusion. For this reason, we only
        var target = null;
        if (lookup != null) {
            if ( lookup._id != null) {
