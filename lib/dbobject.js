...
DbObjectType.create = function(fullOptions) {
    ...
    if ( dbCollection != null) {
        ...
        var databaseExtensions = {
             ...
             * @param changedValues
             */
            updateBy:function(selector, changedValues, options) {
            ...
            updateAllBy:function(selector, changedValues) {
                var result = this.updateBy(selector, changedValues, {multi:true});
                return result;
            },
            updateOneBy:function(selector, changedValues) {
                var result = this.updateBy(selector, changedValues);
                return result;
            },
...
DbObjectType.prototype = {
    ...
    upsertFromUntrusted: function(optionsParam) {
        ...
        if (options.revision) {
            // NOTE(dmr, 2015-04-09) Per discussion w/pat, we force onlyReviseHead to false pending
            // a more careful reflection on exactly when we should enforce onlyReviseHead.
            target._revisionSave(false && options.revisionHeadOnly);
        } else {
            target._save();
        }
        return target;
