 ...
 * @param properties - declare the properties the EJSON code will care about. can be
...
DbObjectType.create = function(options) {
    ...
    if (!_.isEmpty(_.omit(options, DbObjectType.CreateKnownOptionKeys))) {
        throw new Meteor.Error(500,
            "invalid option(s) to DbObjectType.create:",
            _.omit(options, DbObjectType.CreateKnownOptionKeys)
        );
    }
    var subClassType;
    var propertyNames = [ ];
    ...
    if ( databaseTableName ) {
        ...
        _.each(convenienceMethodNames, function(name) {
            convenienceMethods[name] = function() {
                return this.databaseTable[name].apply(this.databaseTable, arguments);
            };
        });
        // In all that noise, we forgot find
        convenienceMethods['find'] = dbCollection.find.bind(dbCollection);
