...
DbObjectType.prototype = {
     ...
     */
    upsertFromUntrusted: function(clientObj, lookup, options) {
        var knownOptionKeys = ['forcedValues', 'frozenKeys', 'updateOnly'];

        if (options == null) {
            options = {};
        } else if (!_.isEmpty(_.omit(options, knownOptionKeys))) {
            throw Error("invalid option to upsertFromUntrusted");
        }
        ...
        if ( target == null ) {
            if (options.updateOnly) {
                throw Error('no update target, but updateOnly=true');
            } else {
            ...
            if (options.frozenKeys && options.frozenKeys.length > 0) {
                clientObj = _.omit(clientObj, _.reject(options.frozenKeys, function(key) {
