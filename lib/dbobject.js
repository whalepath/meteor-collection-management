 ...
 *
 * If the propertyNamesParam contains a property named 'propertyName' with 'indexed' field set to 'true' then the generated collection has 2 additional methods:
 ...
 *
 * Referencing properties in the future will get convinence methods to get to the referenced object. The name will be important.
 *
 * INDEXING
...
DbObjectType.createSubClass = function(subClassTypeParam, typeNameParam, propertyNamesParam, databaseTableNameParam, extensionsParam) {
    ...
    _.extend(subClassType.prototype, {
        constructor: subClassType,
        fromJSONValue: function (rawJSON) {
            var result = DbObjectType.prototype.fromJSONValue.call(this, rawJSON, subClassType.prototype.specialHandling);
            return result;
        },
        toJSONValue: function(rawJSON) {
            var result = DbObjectType.prototype.toJSONValue.call(this, subClassType.prototype.specialHandling);
            return result;
        }
    });
...
DbObjectType.prototype = {
    ...
    toJSONValue: function(specialHandling) {
        var that = this;
        var rawJson = {};
        this.propertyNames.forEach(function(propertyName) {
            switch( typeof(that[propertyName])) {
            case 'undefined':
            case 'function':
                return;
            default:
                ...
                if ( ! jsonFn ) {
                    var jsonHelper = _.deep(specialHandling, propertyName + '.jsonHelper');
                    if (jsonHelper != null) {
                        if (typeof jsonHelper === 'string') {
                            jsonHelper = (window || GLOBAL)[jsonHelper];
                        }
                ...
                if ( jsonFn ) {
                    rawJson[propertyName] = jsonFn(that[propertyName]);
                } else {
                    rawJson[propertyName] = that[propertyName];
                }
            }
        });
        return rawJson;
    },
    fromJSONValue: function(rawJson, specialHandling) {
        var that = this;
        this.propertyNames.forEach(function(propertyName) {
            ...
            default:
                ...
                if ( jsonFn ) {
                    that[propertyName] = jsonFn(rawJson[propertyName]);
                } else {
     ...
     * @param lookup (optional) either mongodb query to find existing
    ...
    upsertFromUntrusted: function(clientObj, lookup, forcedValues) {
        ...
        // We are being called as an instance method by an
        // already-located object.
        if ( _.has(this, '_id') ) {
            target = this;
            // Don't need to find anything.
            lookup = null;
        }
    ...
    _save: function() {
        ...
        if ( this._id ) {
            var affectedDocumentCount = this.databaseTable.update({_id: this._id}, this.toJSONValue(), function(error, affectedDocuments) {
                if (error) {
                    console.log("Failed to update with "+that._id+ ": "+error);
                }
            });
            if ( affectedDocumentCount == 0 ) {
                console.log("No documents found with id:"+that._id);
            }
        } else {
            ...
            var _id = this.databaseTable.insert(jsonValue, function(error, idSet) {
                if ( error ) {
                    console.log("Failed to insert with : "+error);
                }
            });
            this._id = _id;
            this._makePropertyImmutable('_id');
 ...
 */
function _valueOrFunctionResult(valueOrFn) {
    var result;
