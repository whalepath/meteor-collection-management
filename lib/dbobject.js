...
DbObjectType.create = function(options) {
    ...
    if ( options.subClassType ) {
        return DbObjectType.createSubClass(
            options.subClassType,
            options.typeName,
            options.properties,
            options.databaseTableName,
            options.extensions,
            options.privateProperties);
    } else {
        return DbObjectType.createSubClass(
            options.typeName,
            options.properties,
...
DbObjectType.createSubClass = function(subClassTypeParam, typeNameParam, propertyNamesParam, databaseTableNameParam, extensionsParam, privatePropertiesParam) {
    ...
    if ( databaseTableName ) {
        ...
        // TODO: just add this to Mongo.Collections base class?
        _.extend(subClassType.databaseTable, {
            ...
            upsertFromUntrusted: subClassType.prototype.upsertFromUntrusted
        });
...
DbObjectType.prototype = {
    ...
    toEJSONString: function() {
        try {
            ...
            console.error("EJSON.stringify failed on a ", this.typeName(), " id=", this.id, " error=", error);
        }
    },
     ...
     *     lookup: (optional) either mongodb query to find existing or
    ...
    upsertFromUntrusted: function(optionsParam) {
        'use strict';
        var knownOptionKeys = ['clientObj', 'lookup', 'forcedValues', 'frozenKeys', 'updateOnly'];
        ...
        if (!_.isEmpty(_.omit(options, knownOptionKeys))) {
            throw new Meteor.Error(500, "invalid option to upsertFromUntrusted: "+ _.keys(_.omit(options, knownOptionKeys)));
        }
