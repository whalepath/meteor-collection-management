        ...
        function handleFunctionDefinition(possibleFunction) {
            ...
            if ( possibleFunction != null ) {
                if (_.isFunction(possibleFunction)) {
                    ...
                    debugger;
                    throw new Meteor.Error(500, definitionName + ": is not a function or object but is a "+typeof possibleFunction);
                }
            }
        ...
        if ( !mergeExistingDefinition ) {
            ...
            if ( suffixKey ) {
                ...
                if (_.isFunction(existingFunction)) {
                    ...
                    // TODO: maybe should be after definitionProcessingFunction() call to allow definitionProcessingFunction()
                    // to supply default functions.

                    // if the _____Method function does not exist because the definition is { method: function() } then
                    // assign the ____Method = definition.method (or) _____Cursor = definition.cursor
                    // this allows ____Method to always be available and makes it easier to have a proper this when called
                    // i.e. thatManager.___Method() has this as thatManager vs. thatManager.___.method()
