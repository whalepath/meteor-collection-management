...
//        var meteorTopicCursorFunction = thatManager.getMeteorTopicCursorFunction(meteorTopicSuffix);
    ...
    fatal: function() {
        var thatManager = this.thatManager;
        thatManager.error.call(thatManager, arguments);
        debugger;
        var message = EJSON.stringify(arguments);
...
ManagerType.createSubClass = function() {
    ...
    if (arguments.length == 0) {
        throw new Meteor.Error(500,'No arguments in call to ManagerType.createSubClass');
    }
    var subClassType, callPrefix,meteorCallDefinitions, meteorTopicDefinitions, meteorTopics, meteorTopicsParam, primaryDbObjectType, properties, extensions;
    var argIndex = 0;
    if (typeof arguments[0] === 'function') {
        ...
        if ( meteorTopicsParam != null && _.isObject(meteorTopicsParam)){
            meteorTopicDefinitions = {};
            // HACK : begin - but putting *Cursor functions on manager ( should not need this at all )
            // except for the need to put the *Cursor functions.
            // but added to the thatManager
            meteorTopics = {};
            _.each(meteorTopicsParam, function(meteorTopicDefinition, cursorName) {
                ...
                if ( meteorTopicDefinition ) {
                    if (_.isFunction(meteorTopicDefinition.cursor)) {
                        meteorTopics[realCursorName] = meteorTopicDefinition.cursor;
                    } else {
                        meteorTopics[realCursorName] = meteorTopicDefinition;
                    }
                    meteorTopicDefinitions[meteorTopicSuffix] = meteorTopicDefinition;
                } else {
                    meteorTopicDefinitions[meteorTopicSuffix] ={};
                }
            });
            // END HACK BLOCK
        } else {
        ...
        subClassType = function() {
            ManagerType.call( this, callPrefix, meteorCallDefinitions, meteorTopicDefinitions);
        };
    }
    ...
    if ( primaryDbObjectType ) {
        Object.defineProperties(subClassType.prototype, {
            ...
            findOne: {
                value: primaryDbObjectType.databaseTable.findOne.bind(primaryDbObjectType.databaseTable),
                writable: false,
            },
            findById: {
                value: primaryDbObjectType.databaseTable.findById.bind(primaryDbObjectType.databaseTable),
                writable: false,
            },
            findOneById: {
                value: primaryDbObjectType.databaseTable.findOneById.bind(primaryDbObjectType.databaseTable),
                writable: false,
            },
        });
    }
