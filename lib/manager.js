...
_.extend(ManagerType.prototype, {
    ...
    createTopics : function(meteorTopicDefinitions) {
        ...
        function(meteorTopicDefinition, meteorTopicSuffix) {
            thatManager.createPublication(meteorTopicDefinition, meteorTopicSuffix);
        }, true, 'cursor');
    },
    /**
    ...
    _getSetting: function(settingKey) {
        var thatManager = this.thatManager;
        var key;
        if ( settingKey.charAt(0) === '.') {
            key = thatManager.callPrefix+settingKey;
        } else {
            key = settingKey;
        }
        return _.deep(Meteor.settings, key);
    },
    /**
    ...
    startup: function(startupFns) {
        ...
        _.each(startupFunctions, function(startupFn) {
            ...
            } else if ( startupFn != null
                     && (typeof startupFn === 'object')
                     && !_.isEmpty(startupFn) ) {
                StartupFunctions.push(startupFn);
            } else {
                throw new Error(
                    'ManagerType.prototype.startup()',
                    'called with something other than an non-empty objects'
                );
            }
        });
    },
