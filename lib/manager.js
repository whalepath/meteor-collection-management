...
ManagerType = function(callPrefix, meteorCallDefinitions, meteorTopicSuffixes) {
    ...
    if (! callPrefix instanceof String ) {
        throw new Meteor.Error(500, "No call prefix supplied");
    }
...
_.extend(ManagerType.prototype, {
    ...
    createMeteorCallMethods : function(meteorCallDefinitions) {
        var thatManager = this;
        handleStringOrObjectDefinition(meteorCallDefinitions, function(meteorCallDefinition, meteorCallNameSuffix) {
            thatManager.createMeteorCallMethod(meteorCallDefinition, meteorCallNameSuffix);
        });
    },
    createTopics : function(meteorTopicDefinitions) {
        var thatManager = this;
        handleStringOrObjectDefinition(meteorTopicDefinitions, function(meteorTopicDefinition, meteorTopicSuffix) {
...
//        args.unshift(now.getUTCFullYear()+'-'+(now.getUTCMonth()+1)+'-'+now.getUTCDate()+'-'+now.getUTCHours()+'-'+now.getUTCMinutes()+':'+now.getUTCSeconds()+' :');
        ...
        (console.debug || console.trace || console.log).apply(console,args);
    },
    /**
