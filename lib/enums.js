...
//        var enumDefinitions = [].concat(Array.prototype.slice(arguments, 0));
        var that = this;
        var byDbCode = {};
        var symbols = [];
            ...
            toJSONValue: {
                ...
                value: function() {
                    ...
                    if (_.isArray(enumValue)) {
                        ...
                        var result = _.map(enumValue, function(element) {
                            return that.toJSONValue.call(element);
                        });
                        return result;
                    ...
                    // if we do toJSONValue.call('foo'), we come here with this set to String('foo')
                    // rather than 'foo'
                    } else if ( typeof enumValue === 'object'
                                && enumValue.constructor.name == 'String') {
                        var symbol = that.enumOf(enumValue.toString());
                        if ( symbol ) {
                            return symbol.dbCode;
                        }
